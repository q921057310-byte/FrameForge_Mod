...
from freecad.frameforge2.extrusions import (
    make_tslot_face,
    make_vslot_face,
    make_profile_face,
    make_yiheda_vslot,
...
class Profile:
    ...
    def on_changed(self, obj, p):
        # Skip recompute during document restore (all properties set one by one)
    ...
    def execute(self, obj):
        ...
        if getattr(self, '_cached_key', None) == cache_key and getattr(self, '_cached_face', None) is not None:
            ...
            if custom_prof is None or custom_prof.Shape is None or custom_prof.Shape.isNull():
                raise FrameForge2Exception("CustomProfile has no valid shape.")
            ...
            if isinstance(sk_shape, Part.Wire):
                ...
                if not wires:
                    raise FrameForge2Exception("CustomProfile Compound has no wires.")
                ...
                raise FrameForge2Exception(f"Unsupported shape type: {sk_shape.TypeId}")
        ...
        if not (getattr(self, '_cached_key', None) == cache_key and getattr(self, '_cached_face', None) is not None):
            _t1 = time.time()
            App.Console.PrintMessage(f"  [cache MISS] {obj.Family} {obj.SizeName} ({(_t1-_t0)*1000:.0f}ms)\n")
            self._cached_key = cache_key
            self._cached_face = p
        else:
            _t1 = time.time()
            App.Console.PrintMessage(f"  [cache HIT]  {obj.Family} {obj.SizeName} ({(_t1-_t0)*1000:.0f}ms)\n")
...
class ViewProviderProfile:
    ...
    def setEdit(self, vobj, mode):
        ...
        if hasattr(self.Object, "CustomProfile") and self.Object.CustomProfile is not None:
            ...
            try:
                import freecad.frameforge2.create_aluminum_profile

                taskd = freecad.frameforge2.create_aluminum_profile.ImportAluminumProfileTaskPanel(
        ...
        import freecad.frameforge2.edit_profile_tool

        taskd = freecad.frameforge2.edit_profile_tool.EditProfileTaskPanel(self.Object)
        if Gui.Control.activeDialog():
            Gui.Control.closeDialog()
        Gui.Control.showDialog(taskd)
        return True
