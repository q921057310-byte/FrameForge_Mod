...
class CreateTrimmedProfileTaskPanel:
    ...
    def add_trimmed_body(self):
        ...
        if not sel or all(len(s.SubElementNames) == 0 for s in sel):
            App.Console.PrintMessage(translate("FrameForge2", "Select at least one face for TrimmedBody\n"))
            return
        ...
        for selObject in sel:
            ...
            if any(existing is obj for existing, _ in self.trimmed_bodies):
                App.Console.PrintMessage(translate("FrameForge2", "Already added: {}").format(obj.Name) + "\n")
                continue
            self.trimmed_bodies.append((obj, ""))
            added += 1

        if added > 0:
            App.Console.PrintMessage(translate("FrameForge2", "Added {} trimmed body(ies)").format(added) + "\n")
    ...
    def add_trimming_bodies(self):
        App.Console.PrintMessage(translate("FrameForge2", "Add Trimming bodies...\n"))
        ...
        for selObject in Gui.Selection.getSelectionEx():
            ...
            if not valid_subs:
                App.Console.PrintMessage(translate("FrameForge2", "Select faces only") + "\n")
                return
            ...
            if all([tb != entry for tb in trimming_boundaries]):
                ...
                App.Console.PrintMessage(
                    translate("FrameForge2", "\tadd trimming body: {} {}").format(selObject.ObjectName, valid_subs) + "\n"
                )

            else:
                App.Console.PrintMessage(translate("FrameForge2", "Already a trimming boundarie for this TrimmedBody\n"))
    ...
    def remove_trimming_bodies(self):
        App.Console.PrintMessage(translate("FrameForge2", "Remove Trimming body\n"))
    ...
    def open(self):
        App.Console.PrintMessage(translate("FrameForge2", "Opening Create Trimmed Profile\n"))
        App.ActiveDocument.openTransaction("Update Trim")
        self.update_view_and_model()

    def reject(self):
        App.Console.PrintMessage(translate("FrameForge2", "Rejecting CreateProfile {}").format(self.mode) + "\n")
    ...
    def accept(self):
        App.Console.PrintMessage(translate("FrameForge2", "Accepting Create Trimmed Profile\n"))
...
class TrimProfileCommand:
    def GetResources(self):
        return {
            ...
            "MenuText": translate("FrameForge2", "Trim Profile"),
            "Accel": "M, T",
            "ToolTip": translate(
                "FrameForge2",
...
Gui.addCommand("FrameForge2_TrimProfiles", TrimProfileCommand())
