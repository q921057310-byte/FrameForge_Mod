...
class ExtrudedCutout:
    def __init__(self, obj, sketch, selected_face):
        ...
        obj.addProperty(
            ...
            "Sketch",
            "ExtrudedCutout",
            translate("FrameForge2", "The sketch for the cut"),
        ).Sketch = sketch
        ...
        obj.addProperty(
            ...
            "ExtrusionLength",
            "ExtrudedCutout",
            translate("FrameForge2", "Length of the extrusion direction 1"),
        ).ExtrusionLength = 500.0
        obj.setEditorMode("ExtrusionLength", 2)  # Hide by default

        # CutType property
        obj.addProperty(
            "App::PropertyEnumeration", "CutType", "ExtrudedCutout", translate("FrameForge2", "Cut type")
        ).CutType = [
            "Through All",
            "Distance",
        ]
    ...
    def _cutout_key(self, fp):
        """Cache key for the cutout operation. Includes all inputs that affect the result."""
        try:
            key_parts = [id(fp.Sketch)]
            # Base object shape — BoundBox captures length changes
            obj = fp.baseObject[0]
            ...
            if s and not s.isNull():
                ...
                key_parts.append(0)
            key_parts.append(fp.baseObject[1][0])  # face name
            key_parts.append(fp.CutType)
            if fp.CutType == "Distance":
                key_parts.append(fp.ExtrusionLength.Value)
    ...
    def execute(self, fp):
        ...
        try:
            ...
            if fp.Sketch is None or fp.baseObject is None:
                raise FrameForge2Exception("Both the Sketch and baseObject properties must be set.")
            ...
            self._cached_shape = cut_shape

            self._update_structure_data(fp)

        except FrameForge2Exception as e:
            App.Console.PrintError(f"Error: {e}\n")
        except Exception as e:
            App.Console.PrintError(f"ExtrudedCutout: unexpected error: {e}\n")
...
class ViewProviderExtrudedCutout:
    ...
    def setEdit(self, vobj, mode):
        ...
        import freecad.frameforge2.create_extruded_cutout_tool

        taskd = freecad.frameforge2.create_extruded_cutout_tool.CreateExtrudedCutoutTaskPanel(self.Object)
