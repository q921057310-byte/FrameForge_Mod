...
from freecad.frameforgemod.ff_tools import ICONPATH, PROFILESPATH, translate
from freecad.frameforgemod.profile import Profile, ViewProviderCustomProfile
...
class ImportAluminumProfileTaskPanel:
    ...
    def _restore_last_selection(self):
        ...
        param = App.ParamGet("User parameter:BaseApp/Preferences/frameforgemod")
    ...
    def _extract_sections(self, filepath):
        return self._extract_via_zip(filepath)
        ...
        try:
            ...
            if lib_doc is not None:
                try:
                    ...
                    App.Console.PrintError(f"Failed to close library doc '{lib_doc.Name}': {ex}\n")
        return results
    ...
    def _reload_modules(self):
        import importlib
        import sys
        names = [m for m in sys.modules if "frameforge" in m or "frameforgemod" in m]
        for name in sorted(names):
            try:
                importlib.reload(sys.modules[name])
            except Exception:
                pass
        App.Console.PrintMessage("frameforgemod modules reloaded\n")
...
class CreateAluminumProfileCommand:
    def GetResources(self):
        return {
            ...
            "MenuText": translate("frameforgemod", "Aluminum Profile Library"),
