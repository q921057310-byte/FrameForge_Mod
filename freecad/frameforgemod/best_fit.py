...
def best_fit_decreasing(l_stock, parts):
    ...
    for p in sorted_parts:
        # pick the stock that leaves the least free space after placement
        fitting_stocks = sorted(stocks, key=lambda s: s.left)
        sorted_stocks = [s for s in fitting_stocks if s.left >= p.cut_size]
