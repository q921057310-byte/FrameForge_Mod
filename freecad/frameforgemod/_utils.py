...
__title__ = "frameforgemod utilities"
__author__ = "frameforgemod contributors"
__license__ = "LGPL 2.1"
__doc__ = "frameforgemod utilities common to all tools."
...
def _register_profile_metadata(obj, pid_readonly=True):
    """Register shared readonly profile metadata and structure properties.
    Call this from __init__ of FeaturePython proxy classes that need
    Profile BOM integration (Profile, TrimmedProfile, ExtrudedCutout,
    EndCap, Gusset, WhistleConnector, etc.).
    """
...
def _extract_index(name):
    """Safely extract the integer index from a subelement name (e.g. 'Edge1' -> 1)."""
        ...
        if obj.getTypeIdOfProperty(prop) == "App::PropertyLinkSub":
            if shape_type in prop_link[1][0]:
                try:  # FC 0.19+
                    ...
                    n = _extract_index(prop_link[1][0])
                    osh = prop_link[0].Shape
                    sh = osh.copy()
                    if sh and (not shape_type == "Vertex") and hasattr(prop_link[0], "getGlobalPlacement"):
                        pl = prop_link[0].getGlobalPlacement()
                        sh.Placement = pl
            ...
            FreeCAD.Console.PrintError("frameforgemod._utils.getShape: wrong property type.\n")
            return None
    else:
...
def is_endcap(obj):
    if obj.TypeId == "Part::FeaturePython":
        if hasattr(obj, "BaseObject") and hasattr(obj, "Thickness") and hasattr(obj, "CapType"):
            return True
    return False
...
def is_gusset(obj):
    if obj.TypeId == "Part::FeaturePython":
        if hasattr(obj, "Face1") and hasattr(obj, "Face2") and hasattr(obj, "Thickness"):
            return True
    return False


def is_whistleconnector(obj):
    if obj.TypeId == "Part::FeaturePython":
        if hasattr(obj, "DrillFace") and hasattr(obj, "ConnectorSize") and hasattr(obj, "OffsetFromEnd"):
            return True
    return False
...
# TODO move this code into TrimmedProfile ?
def get_children_from_trimmedbody(obj):
    if obj is None:
        return
    yield obj
    if is_trimmedbody(obj):
        yield from get_children_from_trimmedbody(obj.TrimmedBody)
...
# TODO move this code into ExtrudedCutOut ?
def get_children_from_extrudedcutout(obj):
    yield obj
    if is_trimmedbody(obj):
        yield from get_children_from_trimmedbody(obj.TrimmedBody)
    elif is_extrudedcutout(obj):
        yield from get_children_from_extrudedcutout(obj.baseObject[0])
...
def get_trimmed_profile_all_cutting_angles(trimmed_profile):
    ...
    def resolve_edge(link):
        ...
        if target is None:
            return None
        return doc.getObject(target[0].Name).getSubObject(_get_subname(target))
    ...
    if trimmed_profile.TrimmedProfileType == "End Trim":
        ...
        if target1 is None:
            return angles
        edge1 = doc.getObject(target1[0].Name).getSubObject(_get_subname(target1))
        bounds_target = []
        for bound in trimmed_profile.TrimmingBoundary:
        ...
        for target in bounds_target:
            if target is None:
                continue
            trimming_boundary_edges.append(doc.getObject(target[0].Name).getSubObject(_get_subname(target)))
...
def length_along_normal(obj):
    ...
    if is_profile(obj):
        ...
        def resolve_edge(link):
            ...
            if isinstance(subname, (list, tuple)):
                subname = subname[0]
            return doc.getObject(target[0].Name).getSubObject(str(subname))

        edge = resolve_edge(obj.TrimmedBody)
        if edge is None:
            if hasattr(obj, 'ProfileLength') and obj.ProfileLength:
                return obj.ProfileLength
            try:
                prof = get_profile_from_trimmedbody(obj)
                if prof is not None and hasattr(prof, 'ProfileLength') and prof.ProfileLength:
                    return prof.ProfileLength
            except Exception:
                pass
            return 0.0
